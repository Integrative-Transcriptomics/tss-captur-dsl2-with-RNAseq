    ...
    for k in range(4, 8):
        for i in range(150):
            ...
            if _y1 > len(sequence):
                ...
                window = str(window)
                gc_content = 100*(window.count("C") +
                                  window.count("G"))/len(window)
                score_p = score
...
try:
    ...
    for sq in sequences_to_analyze:
        ...
        predictions_file = open(
            f'predictions_coordinates_{args.output}.tsv', mode='a')
        writer = csv.writer(predictions_file, delimiter='\t')
        writer.writerow(['Region', 'Start RUT', 'End RUT', 'Strand', "score"])
        p = open(f"info_about_predictions_{args.output}.txt", "a")
                ...
                if c_over_g > 1:
                    if re.search(pattern1, w):
                        ...
                        if ctrl > 0 or re.search(pattern_pause_site1, s):
                            ...
                            p.write("\n")
                            cod += 1
                            final_score = palindrome_finder(
                                s, p, gc_whole_genome, 1, score, calc="yes")
                            start = 0
                            while True:
                                if re.search(pattern_pause_site1, s):
                                    research = re.search(
                                        pattern_pause_site1, s)
                            ...
                            writer.writerow(
                                [f'T{fix_cod}', x1, fix_x2, '+', final_score])
