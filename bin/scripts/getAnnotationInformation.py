...
startlist = []  # list of sequence identifiers that will be sent to GenBank Nucleotide
endlist = []  # list of sequence identifiers that will be sent to GenBank Nucleotide
strandlist = []  # list of sequence identifiers that will be sent to GenBank Nucleotide
...
for line in lines:
    ...
    if len(line) > 1:  # condition to deal with blank lines
        idlist.append(re.findall(".*\|.*\|.*\|(.*)\|", line[1])[0])
        startlist.append(line[2])
        endlist.append(line[3])
        strandlist.append(line[5])
print(idlist[0:10])
...
def get(ids, starts, ends, strands, step):
    handle = Entrez.efetch(
        db=dbdict[db], id=ids, strand=strands, seq_start=starts, seq_end=ends,  retmode='xml')
...
try:
    for i in range(len(idlist)//200):
        ...
        starts = ",".join(startlist[a:b])
        ends = ",".join(endlist[a:b])
        strands = ",".join(strandlist[a:b])
        get(ids_to_fetch, starts, ends, strands, step)
        a += 200
        b += 200
        step += 1
    ids_to_fetch = ",".join(idlist[a:])
    starts = ",".join(startlist[a:])
    ends = ",".join(endlist[a:])
    strands = ",".join(strandlist[a:])
    if ids_to_fetch != "":
        get(ids_to_fetch, starts, ends, strands, step)
    ...
    # list the ID numbers of current step - e.g. 201-400
    print(
        f"RuntimeError occurred during step {step-1}. Trying one more time...")
